...
function createTray() {
    ...
    const useTraditionalChinese = locale === "zh-tw" || locale === "zh-hk" || locale.startsWith("zh-hant")
    const trayLabels = useTraditionalChinese
        ? {
            toggleVisibility: "切換顯示",
            removeFromDock: "從 Dock 移除",
            quit: "結束 Seanime",
        }
        : {
    ...
    const contextMenu = Menu.buildFromTemplate([{
        id: "toggle_visibility", label: trayLabels.toggleVisibility, click: () => {
            if (!serverStarted) return
            if (mainWindow.isVisible()) {
                hideMainWindow()
            } else {
        ...
        id: "accessory_mode", label: trayLabels.removeFromDock, click: () => {
            app.dock.hide()
        }
    }
    ] : []), {
        id: "quit", label: trayLabels.quit, click: () => {
            cleanupAndExit()
        }
    }
    ])
