...
export function AutoSelectProfileButton() {
    ...
    const getPreferenceLabel = React.useCallback((value: Anime_AutoSelectPreference) => {
        return t(`settings.autoSelect.preferenceOptions.${value}`)
    }, [t])
    ...
    return (
        <>
                ...
                {hasProfile && (
                    <div className="text-sm text-[--muted] space-y-1">
                        {profile.resolutions && profile.resolutions.length > 0 && (
                            <p><strong>{t("settings.autoSelect.summary.resolutions")}:</strong> {profile.resolutions.join(", ")}</p>
                        )}
                        {profile.releaseGroups && profile.releaseGroups.length > 0 && (
                            <p><strong>{t("settings.autoSelect.summary.groups")}:</strong> {profile.releaseGroups.slice(0, 3).join(", ")}{profile.releaseGroups.length > 3 ? "..." : ""}
                            </p>
                        )}
                        {profile.providers && profile.providers.length > 0 && (
                            <p><strong>{t("settings.autoSelect.summary.providers")}:</strong> {profile.providers.join(", ")}</p>
                        )}
                        {profile.preferredCodecs && profile.preferredCodecs.length > 0 && (
                            <p><strong>{t("settings.autoSelect.summary.preferredCodecs")}:</strong> {profile.preferredCodecs.join(", ")}</p>
                        )}
                        {profile.preferredSources && profile.preferredSources.length > 0 && (
                            <p><strong>{t("settings.autoSelect.summary.preferredSources")}:</strong> {profile.preferredSources.join(", ")}</p>
                        )}
                        {profile.preferredLanguages && profile.preferredLanguages.length > 0 && (
                            <p><strong>{t("settings.autoSelect.summary.preferredLanguages")}:</strong> {profile.preferredLanguages.join(", ")}</p>
                        )}
                        {profile.multipleAudioPreference && profile.multipleAudioPreference !== "neutral" && (
                            <p><strong>{t("settings.autoSelect.summary.multipleAudio")}:</strong> {getPreferenceLabel(profile.multipleAudioPreference)}</p>
                        )}
                        {profile.multipleSubsPreference && profile.multipleSubsPreference !== "neutral" && (
                            <p><strong>{t("settings.autoSelect.summary.multipleSubtitles")}:</strong> {getPreferenceLabel(profile.multipleSubsPreference)}</p>
                        )}
                        {profile.bestReleasePreference && profile.bestReleasePreference !== "neutral" && (
                            <p><strong>{t("settings.autoSelect.summary.bestReleases")}:</strong> {getPreferenceLabel(profile.bestReleasePreference)}</p>
                        )}
                        {profile.batchPreference && profile.batchPreference !== "neutral" && (
                            <p><strong>{t("settings.autoSelect.summary.batches")}:</strong> {getPreferenceLabel(profile.batchPreference)}</p>
                        )}
                    </div>
                )}
            </div>
            ...
            <Modal
                ...
                title={t("settings.common.autoSelect")}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border rounded-[--radius] p-4 relative space-y-3">
                <div className="absolute -top-2.5 tracking-wide font-semibold uppercase text-sm left-4 bg-gray-950 px-2">
                    {t("settings.common.releasePreferences")}
                </div>
            ...
            <div className="border rounded-[--radius] p-4 relative space-y-3">
                <div className="absolute -top-2.5 tracking-wide font-semibold uppercase text-sm left-4 bg-gray-950 px-2">
                    {t("settings.common.metadataPreferences")}
                </div>
                ...
                <div className="flex items-center gap-2">
                    <Checkbox
                        ...
                        label={t("settings.autoSelect.requireLanguageMatch")}
                        help={t("settings.autoSelect.requireLanguageMatchHelp")}
                    />
                </div>
                ...
                <div className="flex items-center gap-2">
                    <Checkbox
                        ...
                        label={t("settings.autoSelect.requireCodecMatch")}
                        help={t("settings.autoSelect.requireCodecMatchHelp")}
                    />
                </div>
                ...
                <div className="flex items-center gap-2">
                    <Checkbox
                        ...
                        label={t("settings.autoSelect.requireSourceMatch")}
                        help={t("settings.autoSelect.requireSourceMatchHelp")}
                    />
                </div>
            </div>
            ...
            <div className="border rounded-[--radius] p-4 relative space-y-3">
                <div className="absolute -top-2.5 tracking-wide font-semibold uppercase text-sm left-4 bg-gray-950 px-2">
                    {t("settings.common.specialPreferences")}
                </div>

                <PreferenceField
                    label={t("settings.autoSelect.multipleAudio")}
                ...
                <PreferenceField
                    label={t("settings.autoSelect.multipleSubtitles")}
                ...
                <PreferenceField
                    label={t("settings.autoSelect.batches")}
                ...
                />
                <PreferenceField
                    label={t("settings.autoSelect.bestReleases")}
                    without={["only"]}
            ...
            <div className="border rounded-[--radius] p-4 relative space-y-3">
                <div className="absolute -top-2.5 tracking-wide font-semibold uppercase text-sm left-4 bg-gray-950 px-2">
                    {t("settings.common.thresholds")}
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="space-y-2">
                        <label className="text-sm font-medium">{t("settings.fields.minSeeders")}</label>
                        <NumberInput
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium">{t("settings.fields.minSize")}</label>
                        <TextInput
                            value={formData.minSize}
                            onChange={(e) => setFormData(draft => {
                                draft.minSize = e.target.value
                            })}
                            placeholder={t("settings.autoSelect.minSizePlaceholder")}
                        />
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-medium">{t("settings.fields.maxSize")}</label>
                        <TextInput
                            value={formData.maxSize}
                            onChange={(e) => setFormData(draft => {
                                draft.maxSize = e.target.value
                            })}
                            placeholder={t("settings.autoSelect.maxSizePlaceholder")}
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-between gap-2">
                {profile && (
                    <Button
                        ...
                        loading={deleting}
                    >
                        {t("settings.autoSelect.resetAll")}
                    </Button>
                )}
...
function ProvidersFieldControlled() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.providersOrderedMax3")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.providersHelp")}</p>
            <Combobox
                value={comboboxValue}
                onValueChange={handleAdd}
                options={availableOptions}
                multiple
                label={t("settings.autoSelect.addProvider")}
                disabled={items.length >= 3}
                emptyMessage={t("settings.autoSelect.noProvidersFound")}
            />
            {items.length > 0 && (
...
function ReleaseGroupsSortableField() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.releaseGroupsOrdered")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.releaseGroupsHelp")}</p>
...
function ResolutionsSortableField() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.resolutionsOrdered")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.resolutionsHelp")}</p>
...
function ExcludeTermsSortableField() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.excludeTerms")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.excludeTermsHelp")}</p>
                    ...
                    <div className="space-y-2">
                        {items.map((item) => (
                            <SortableTextItem
                                ...
                                id={item.id}
                                value={item.value}
                                placeholder={t("settings.autoSelect.excludeTermsPlaceholder")}
                                onUpdate={handleUpdate}
                                onRemove={handleRemove}
                            />
                        ))}
...
function PreferredLanguagesSortableField() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.preferredLanguagesOrdered")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.preferredLanguagesHelp")}</p>
...
function PreferredCodecsSortableField() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.preferredCodecsOrdered")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.preferredCodecsHelp")}</p>
...
function PreferredSourcesSortableField() {
    ...
    return (
        <div className="space-y-2">
            <label className="text-sm font-medium">{t("settings.fields.preferredSourcesOrdered")}</label>
            <p className="text-sm text-[--muted]">{t("settings.autoSelect.preferredSourcesHelp")}</p>
...
function PreferenceField(props: {
    ...
    const options = [
        { label: t("settings.autoSelect.preferenceOptions.neutral"), value: "neutral" },
        { label: t("settings.autoSelect.preferenceOptions.prefer"), value: "prefer" },
        { label: t("settings.autoSelect.preferenceOptions.avoid"), value: "avoid" },
        { label: t("settings.autoSelect.preferenceOptions.only"), value: "only" },
        { label: t("settings.autoSelect.preferenceOptions.never"), value: "never" },
