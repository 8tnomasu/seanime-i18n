...
export function AutoDownloaderProfiles() {
    ...
    return (
        <div className="pt-4">
                ...
                <div className="space-y-2">
                    ...
                    {!profiles?.length && (
                        <div className="p-4 text-center text-[--muted]">
                            {t("autoDownloader.profiles.empty")}
                        </div>
                    )}
                </div>
            </Card>
            ...
            <Modal
                title={t("autoDownloader.profiles.createTitle")}
                open={isCreateOpen}
            ...
            <Modal
                title={t("autoDownloader.profiles.editTitle", { name: selectedProfile?.name })}
...
function ProfileItem({ profile, onEdit }: { profile: Anime_AutoDownloaderProfile, onEdit: () => void }) {
    ...
    const confirmDialog = useConfirmationDialog({
        title: t("autoDownloader.profiles.deleteTitle"),
        description: t("autoDownloader.profiles.deleteDescription", { name: profile.name }),
                    ...
                    <div className="w-full">
                        ...
                        <div className="text-sm text-[--muted] line-clamp-1 gap-2 space-x-3">
                            ...
                            {!!profile.conditions?.length &&
                                <span>{t("autoDownloader.profiles.conditionsCount", { count: profile.conditions?.length })}</span>}
                            {!!profile.minimumScore && <span>{`>=`} {profile.minimumScore}</span>}
                            {!!profile.delayMinutes && <span>{t("autoDownloader.profiles.delayMinutes", { count: profile.delayMinutes })}</span>}
